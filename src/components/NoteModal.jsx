...
export default function NoteModal({ note, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    // Sync items pour les listes todo
    if (form.type === 'todo' && noteId) {
      // Supprimer les anciens et réinsérer (plus simple)
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ background: form.color }}>
        <div className="modal-header" style={{ background: form.color, borderBottomColor: 'rgba(0,0,0,0.1)' }}>
          <h2>{isNew ? 'Nouvelle note' : isOwner ? 'Note' : 'Note (partenaire)'}</h2>
        ...
        <div className="modal-body" style={{ background: form.color }}>
          <input
            ...
            disabled={!isOwner}
            style={{ width: '100%', border: 'none', background: 'transparent', fontSize: '1.1rem', fontWeight: 600, padding: '0.5rem 0', outline: 'none' }}
          />
          ...
          {isOwner && (
            <>
              <div style={{ marginTop: '1rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 500 }}>Couleur</label>

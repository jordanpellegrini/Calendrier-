...
import { scheduleEventReminder, rescheduleAllReminders } from '../lib/notifications'
...
export default function Calendar() {
  ...
  return (
    <>
      ...
      {view === 'agenda' && (
        <AgendaView events={events} onEventClick={setEditingEvent} />
      )}
...
function MonthView({ currentDate, setCurrentDate, monthGrid, weekStartsOn, getEventsForDay, onDayClick }) {
          ...
          return (
            ...
            >
              ...
              {dayEvents.slice(0, 3).map(e => (
                <div key={e.id} className="event-pill" style={{ background: e.color }}>
...
function WeekView({ currentDate, setCurrentDate, weekStartsOn, getEventsForDay, onEventClick, onDayClick }) {
            ...
            <div key={d.toISOString()} className="day-view" style={{ cursor: 'pointer' }} onClick={() => onDayClick(d)}>
              ...
              <div className="day-events">
                {dayEvents.length === 0 ? (
                  ...
                  dayEvents.map(e => (
                    <div key={e.id} className="event-item" style={{ borderLeftColor: e.color }} onClick={(ev) => { ev.stopPropagation(); onEventClick(e) }}>
...
function DayView({ currentDate, setCurrentDate, events, onEventClick }) {
  return (
    <>
      <div className="calendar-header">
      ...
      <div className="day-view">
        <div className="day-events" style={{ maxHeight: 'none' }}>
          {events.length === 0 ? (
            ...
            events.map(e => (
              <div key={e.id} className="event-item" style={{ borderLeftColor: e.color }} onClick={() => onEventClick(e)}>
                <div style={{ flex: 1 }}>
